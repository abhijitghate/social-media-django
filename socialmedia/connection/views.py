...
class MyFriendsView(APIView):
    ...
    def get(self, request):
        ...
        queryset = FriendRequest.objects.filter(
            sent_by__email=request.user, status="accepted"
        )
...
class FriendRequestCreateView(APIView):
    ...
    def post(self, request):
        ...
        if existing_friend_requests:
            return Response(
                {
                    "message": "You are already friends or there is a active friend request"
                }
            )
    ...
    def put(self, request, pk):
        ...
        try:
            ...
            if (
                ...
                and user.id != friend_request.sent_to.id
            ):
                return Response(
                    {
                        "error": "Friend Request can be accepted or rejected by someone who it is sent to"
                    }
                )
            if new_status == "canceled" and user.id != friend_request.sent_by.id:
                return Response(
                    {
                        "error": "Friend Request can be canceled by someone who it is sent by"
                    }
                )
        except FriendRequest.DoesNotExist:
            return Response(
